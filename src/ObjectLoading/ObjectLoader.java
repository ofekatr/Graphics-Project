...
public class ObjectLoader {
    ...
    public ArrayList<float[]> getvData() {
        return vData;
    }
    ...
    private Collidable createCollidable() {
        ...
        for (float[] point : this.vData) {
            if (point[0] > maxVals.getX())
                maxVals.setX(point[0]);
            if (point[0] < minVals.getX())
                minVals.setX(point[0]);
            if (point[1] > maxVals.getY())
                maxVals.setY(point[1]);
            if (point[1] < minVals.getY())
                minVals.setY(point[1]);
            if (point[2] > maxVals.getZ())
                maxVals.setZ(point[2]);
